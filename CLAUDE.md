...
- `models/face/` — EAR / blink / PERCLOS / gaze / head pose from one MediaPipe inference.
  **Implemented** (core suite + demo `run.py`). Adopted 2026-09-20 from
...
Verify face with `cd models/face && docker compose run --rm verify` (or
`pip install -e ".[dev]" && pytest`).

Key invariants across models: models emit scores + quality, **fusion owns alert decisions**; bad
input yields null / `UNAVAILABLE` output, never a guess; quality gates outputs but is never a model
feature; training deps stay out of the runtime path; resource budgets are validated with all models
running concurrently on the Pi.
